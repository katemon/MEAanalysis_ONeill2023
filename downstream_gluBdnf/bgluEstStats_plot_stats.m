%% part 2a - BGLU - plot est stats by subtracting from control

clc; clear all; close all;

theDir = 'D:\kate_dropbox\Dropbox\Rutgers\Firestein Lab\Manuscript DRAFTS\2022 BDNF MEA paper\figures\data - BDNF GLU injury recovery\revision_0g30gonly_newPlots\data\';%C:\Users\kmfo8\Dropbox
figDir = 'D:\kate_dropbox\Dropbox\Rutgers\Firestein Lab\Manuscript DRAFTS\2022 BDNF MEA paper\figures\data - BDNF GLU injury recovery\revision_0g30gonly_newPlots\data\figs_estStats\';%C:\Users\kmfo8\Dropbox
...
% yL1 = [-20  -1.75 -350 -1 -10];
% yL2 = [ 5    0.75  500 0.5   10];
% yR1 = [-1.1 -0.11 -100 -0.1  -1];
% yR2 = [ 0.6  0.06  200 0.05    1];
% yLtl1 = [-1];
% yLtl2 = [1];
...
%     disp([thisVar, ' 72h post vs pre, diff in % change']);
    ...
    %         X1 = (().*100;div17_0g_0b - div15_0g_0b)./div15_0g_0b).*100;
    %     end %if strcmp
    n1 = length(X1);
    n2 = length(X2);
    myStatistic = @(X1,X2) mean(X2)-mean(X1);
...
% save([figDir,'estStats_ff100msec.mat'], 'wthnStats','btwnStats');

%% part 2b - BGLU - plot est stats by subtracting within injury conditions
...
% yL1 = [-20  -1.75 -350 -1 -10];
% yL2 = [ 5    0.75  500 0.5   10];
% yR1 = [-0.6 -0.06 -300 -0.07  -1];
% yR2 = [ 0.6  0.06  200 0.05    1];
% yLtl1 = [-1];
% yLtl2 = [1];

for ii=5%1:length(varNames)
...
%     disp([thisVar, ' 72h post vs 24h post, diff in % change']);
    ...
    if ready2save
        ...
        saveas(fig2,[figDir,thisVar,'_withinInjury_div15div14.png']);
        saveas(fig2,[figDir,thisVar,'_withinInjury_div15div14.fig']);
        saveas(fig2,[figDir,thisVar,'_withinInjury_div15div14.svg']);
        
        saveas(fig3,[figDir,thisVar,'_withinInjury_div17div14.png']);
        saveas(fig3,[figDir,thisVar,'_withinInjury_div17div14.fig']);
        saveas(fig3,[figDir,thisVar,'_withinInjury_div17div14.svg']);
        
        saveas(fig4,[figDir,thisVar,'_withinInjury_div17div15.png']);
        saveas(fig4,[figDir,thisVar,'_withinInjury_div17div15.fig']);
        saveas(fig4,[figDir,thisVar,'_withinInjury_div17div15.svg']);
    end %if ready2save
...
% save([figDir,'estStats_ff100msec_withinInjury.mat'], ...
%     'wthnStats','btwnStats');

%% part 2c - BGLU - plot only the distributions (SUBTR FROM CONTROL)
clc; clear all; close all;
...
% y1 = [-1.1 -0.11 -150 -0.1  -1];
% y2 = [ 0.6  0.06  200 0.05   1];
...
for ii=1:length(varNames)
    ...
    try
        saveas(fig1,[theDir,thisVar,'_minus0g0B.png']);
    catch
        disp(['could not save fig1 ',thisVar,'as *.png']);
    end
    saveas(fig1,[theDir,thisVar,'_minus0g0B.fig']);
    saveas(fig1,[theDir,thisVar,'_minus0g0B.svg']);
